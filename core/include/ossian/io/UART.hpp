...
namespace ossian
{
	...
	public:
		...
		UART() = delete;
		UART(std::string location, Baudrate baudrate, FlowControl flowctrl, DataBits databits, StopBits stopbits, Parity parity) noexcept :m_Location(location)
		{
			Open(location, baudrate, flowctrl, databits, stopbits, parity);
		}
		...
		void Open(std::string location, Baudrate baudrate, FlowControl flowctrl, DataBits databits, StopBits stopbits, Parity parity)
		{
			std::string devLocation = "/dev/" + location;
			struct termios opt;
			...
			SetFlag(opt.c_cflag, databits); // 数据位设置
			switch (stopbits)
			{
			case UART::StopBits1:
		...
		FrameData Read()
		{
			FrameData frameData = ReadUARTRaw();//raw frame
		...
		void WriteUARTRaw(unsigned int id, size_t length, uint8_t* data)
		{
			while (1)
			{
	...
	public:
		...
		void SendToRaw(std::string location, unsigned int id, size_t length, uint8_t* data)
		{
			...
			dev->WriteUARTRaw(id, length, data);
		}
		// 从指定位置读取FrameData
		...
		{
			auto device = std::make_unique<UART>(location, baudrate, flowctrl, databits, stopbits, parity);
		...
		UART* FindDevice(std::string location)
		{
			...
			if (m_DevicesMap.end() == it)
			{
				throw std::runtime_error("No such UART device");
			}
			return it->second.get();
		}
	private:
		std::unordered_map<std::string, std::unique_ptr<UART>> m_DevicesMap;
	};
} //NautilusVision
